...
btnVerLista.addEventListener("click", async function (){
    ...
    await axios.post('https://appit.itsup.edu.ec/postgrado/estudiante/findestudianteidcarreracalificacion', {
        idperiodo,
        idcarrera
      })
    .then(function (res) {
...
async function verCalificaciones(estudiante){
    ...
    await axios.post('https://appit.itsup.edu.ec/postgrado/calificaciones/findcalificacionesidmatricula', {
        idmatricula
    })
    .then(function (res) {
        ...
        document.getElementById("calificacion").innerHTML = calificaciones;
    })
    .catch(function (error) {
        $.confirm({
            title: 'Acceso no autorizado',
