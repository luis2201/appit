<?php
    ...
    class Resumen extends DB
    {
        public static function findMateriasIdEstudiante($params)
        {
            ...
            $prepare = $db->prepare("call sp_materias_select_idmatricula(:idperiodo, :idmatricula);");
            $prepare->execute($params);

            return $prepare->fetchAll(PDO::FETCH_CLASS, Resumen::class);
        }

        public static function findAll_ID($idestudiante, $idperiodo, $idparcial)
        {
            $db = new DB();
            $prepare = $db->prepare("call 	sp_calificaciones_estudiante_ID(:idestudiante, :idperiodo, :idparcial);");
            $prepare->execute([":idestudiante" => $idestudiante, ":idperiodo" => $idperiodo, ":idparcial" => $idparcial]);
