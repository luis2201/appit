...
async function parcial1()
{
    ...
    await axios.post(DIR + 'notas/findallid/', {
        idperiodo,
        idparcial,
        idestudiante
    })
    .then(function (res) {
        ...
        for (let i = 0; i < info.length; i++) {
            ...
            fila.insertCell().innerHTML = info[i]['practica'];
            fila.insertCell().innerHTML = "<b>"+info[i]['tpractica']+"</b>";
            fila.insertCell().innerHTML = info[i]['aprendizaje'];
            fila.insertCell().innerHTML = "<b>"+info[i]['taprendizaje']+"</b>";
            fila.insertCell().innerHTML = info[i]['resultado'];
            fila.insertCell().innerHTML = "<b>"+info[i]['tresultado']+"</b>";
