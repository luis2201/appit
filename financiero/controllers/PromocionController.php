<?php
  ...
  class PromocionController
  {
    ...
    public function viewcalificacion()
    {
      ...
      $resp = Promocion::findIdMatricula([":idperiodo" => $idperiodo, ":idestudiante" => $idestudiante]);
      foreach ($resp as $row) {
        $idmatricula = $row->idmatricula;
      }

      $resp = Promocion::findNivelCarrera([":idperiodo" => $idperiodo, ":idestudiante" => $idestudiante]);
      foreach ($resp as $row) {
        $nivel = $row->nivel;
        $carrera = $row->carrera;
      }
      ...
      echo json_encode(["nivel" => $nivel, "carrera" => $carrera, "rows" => $rows]);
    }
    ...
    public function viewcalificacionenlinea()
    {
        ...
        foreach($resp as $materias)
        {        
          $resp = Promocion::findParcialVirtual([":idmatricula" => $idmatricula, ":idmateria" => $materias->idmateria, ":idparcial" => 1]);
          foreach ($resp as $parcial1) {
            $p1 = ($parcial1->total>0)?number_format($parcial1->total, 2):"";
          }
  
          $resp = Promocion::findParcialVirtual([":idmatricula" => $idmatricula, ":idmateria" => $materias->idmateria, ":idparcial" => 2]);
          foreach ($resp as $parcial2) {
            $p2 = ($parcial2->total>0)?number_format($parcial2->total, 2):"";
            $supletorio = ($parcial2->supletorio>0)?number_format($parcial2->supletorio, 2):"";
          }
