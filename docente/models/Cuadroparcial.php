<?php
    ...
    class Cuadroparcial extends DB
    {
        public static function findMaterias($idperiodo, $idcarrera, $idnivel)
        {
            ...
            $prepare = $db->prepare("SELECT T.idmateria, T.codigo, T.materia
                                    FROM tb_materia T
                                        INNER JOIN tb_detalle_matricula D ON T.idmateria = D.idmateria
                                        INNER JOIN tb_matricula M ON M.idmatricula = D.idmatricula
            ...
            return $prepare->fetchAll(PDO::FETCH_CLASS, Cuadroparcial::class);
        }
