<?php
  ...
  class RegistrocalificacionController
  {
    ...
    public function index()
    {
      ...
      view("registrocalificacion.index", ["carreras" => $carreras]);
    }
    ...
    public function viewlistaestudiantemateria()
    {
      ...
      foreach ($res as $row) {
        $rows .= '<tr style="height: 20px; font-size:0.8vw">
        ...
        $calificaciones = Registrocalificacion::findcalificacionidmatricula($row->idmatricula, $idperiodo, $idmateria, $idparcial);
        if(count($calificaciones)>0){
          foreach ($calificaciones as $cal) {               
            ...
            $practicas = (!is_null($cal->practicas))?number_format($cal->practicas,2):number_format(0,2); 
            $tpracticas = (!is_null($cal->tpracticas))?number_format($cal->tpracticas,2):number_format(0,2); 
            ...
            $resultados = (!is_null($cal->resultados))?number_format($cal->resultados,2):number_format(0,2); 
            $tresultados = (!is_null($cal->tresultados))?number_format($cal->tresultados,2):number_format(0,2); 
    ...
    public function findcalificacionidmatricula()
    {
      ...
      $res = Registrocalificacion::findcalificacionidmatricula($idmatricula, $idperiodo, $idmateria, $idparcial);
