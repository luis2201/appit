<?php
    ...
    class AsistenciadocenteController
    {
        ...
        public function viewlistaestudiantes()
        {
            ...
            $resp = Materia::findDiasLaborales($params);
            $cols = count($resp);

            $table = '';
      
            $thead1 = '<table id="tbLista" class="table table-striped" cellspacing="0" cellpadding="2" style="font-size:0.6em; margin-top: -20px; width:100%;">
            ...
            $resp = Asistenciadocente::ViewListaEstudiantes($params);
            $n=0;
            $i=1;
            foreach ($resp as $row) {
                $params = [":idmateria" => $idmateria, ":idmatricula" => $row->idmatricula];
                $asistencias = Asistenciadocente::findIdMatriculaFecha($params);
                
                $row1 = '<tr>
                            <td class="text-center">'.$i++.'</td>
                            <td class="text-center">'.$row->idmatricula.'</td>
                            <td style="width: 35%;">'.$row->estudiante.'</td>';
